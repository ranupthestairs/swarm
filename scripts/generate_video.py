...
def _workspace_submission_dir(extracted: Path | None = None) -> Path:
    """Return a writable path that mirrors Docker's /workspace/submission layout."""
    override = os.environ.get("SWARM_VIDEO_WORKSPACE")
    if override:
        ws = Path(override)
        ws.mkdir(parents=True, exist_ok=True)
        return ws

    docker_ws = Path("/workspace/submission")
    try:
        docker_ws.mkdir(parents=True, exist_ok=True)
        return docker_ws
    except OSError:
        pass

    if extracted is not None:
        fallback = extracted / ".workspace"
    else:
        import tempfile

        fallback = Path(tempfile.gettempdir()) / "swarm_workspace" / "submission"
...
def _link_workspace(extracted: Path) -> None:
    ...
    ws = _workspace_submission_dir(extracted)
